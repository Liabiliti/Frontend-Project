...
<body>
  ...
  <form action="create.php?data=" class="form">
    <h1 class="form__title">Medicine and Child</h1>
    <p class="form__description">Create, read, update and delete medicine and child information</p>

  <div class="form__group">
    <select class="form__input" id="ChildID" name="ChildID" required placeholder=" " autocomplete="off">
      <?php
...
<div class="form__group">
  <select class="form__input" id="MedicineID" name="MedicineID" required placeholder=" " autocomplete="off">
    <?php
    include 'connection.php';
    $sql = "SELECT MedicineID FROM `Medicine`";
    $result = $conn->query($sql);
    while($row = $result->fetch_assoc()) {
      echo "<option value='".$row['MedicineID']."'>".$row['MedicineID']."</option>";
    }
    ?>
  </select>
  <label for="MedicineID" class="form__label">Medicine</label>
</div>
<div class="form__group">
  <input type="number" id="Dosage"  name="Dosage" class="form__input" placeholder=" " required autocomplete="off" min="0" max="100" ></input>
  <label for="Dosage" class="form__label">Dosage</label>
</div>
